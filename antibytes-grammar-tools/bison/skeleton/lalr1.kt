# Kotlin skeleton for Bison                           -*- kotlin -*-
...
m4_include(b4_skeletonsdir/[kotlin.m4])

b4_header_if([b4_complain([%header/%defines does not make sense in Kotlin])])

m4_define([b4_symbol_no_destructor_assert],
[b4_symbol_if([$1], [has_destructor],
[b4_complain_at(m4_unquote(b4_symbol([$1], [destructor_loc])),
[%destructor does not make sense in Kotlin])])])
...
]b4_copyright([Skeleton implementation for Bison LALR(1) parsers in Kotlin],
[2007-2015, 2018-2021])[
]b4_disclaimer[
...
]b4_parser_class_declaration[
{
    ...
    // TODO Replace with Interface for a KMP PrintStream
    ...
    {]b4_locations_if([[
        ...
        for (;;)
            labelSwitch: switch (label)
            {
                ...
                /* Accept?  */
                if (yystate == YYFINAL_) {
                    ]b4_push_if([label = YYACCEPT; break labelSwitch;],
                    [return true;])[
                }
                ...
                if (yyerrstatus_ == 3)
                {
                    ...
                    if (yychar <= Lexer.]b4_symbol(eof, id)[) {
                        ...
                        if (yychar == Lexer.]b4_symbol(eof, id)[) {
                            ]b4_push_if([label = YYABORT; break labelSwitch;], [return false;])[
                        }
                    } else {
                        yychar = YYEMPTY_;
                    }
        ...
        if (!yyPactValueIsDefault(yyn))
        {
            ...
            for (int yyx = yyxbegin; yyx < yyxend; ++yyx) {
                if ( yycheck_[yyx + yyn] == yyx &&
                    yyx != ]b4_symbol(error, kind)[.getCode() &&
