...
abstract class BisonTask : BisonTaskContract, DefaultTask() {
    ...
    private fun addErrorOptions(arguments: MutableList<String>) {
        var warnings = if (warnings.get().isNotEmpty()) {
            warnings.get().joinToString(",") { option -> option.value }
        } else {
            ""
        }

        if (errors.get().isNotEmpty()) {
            val errors = errors.get().joinToString(",") { option -> "error=${option.value}" }
            warnings += ",$errors"
        }

        arguments.addIf(warnings.isNotEmpty()) { "--warning=${warnings.trimStart(',')}" }
    }
