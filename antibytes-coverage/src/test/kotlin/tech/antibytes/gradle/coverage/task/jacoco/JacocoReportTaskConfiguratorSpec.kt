...
class JacocoReportTaskConfiguratorSpec {
    ...
    fun `Given configure is called with a Project, ContextName and Configuration, it adds a CoverageTask for JVM and returns it`() {
        ...
        invokeGradleAction(
            { probe -> tasks.create("${contextName}Coverage", JacocoReport::class.java, probe) },
            jacocoTask,
            jacocoTask
        )
        ...
        every {
            fileTreeExecutionFiles.setIncludes(
                testDependencies.map { name -> "jacoco${File.separator}$name.exec" }.toSet()
            )
        } returns mockk()
        ...
        every {
            buildDirLayout
                .dir("reports/jacoco/$contextName/$projectName")
                .get()
                .asFile
        } returns htmlDir
        ...
        every {
            buildDirLayout
                .file("reports/jacoco/$contextName/$projectName.csv")
                .get()
                .asFile
        } returns csvFile
        ...
        every {
            buildDirLayout
                .file("reports/jacoco/$contextName/$projectName.xml")
                .get()
                .asFile
        } returns xmlFile
        ...
        verify(exactly = 1) { jacocoTask.description = "Generate coverage reports for ${contextName.capitalize()}." }
    ...
    @Test
    fun `Given configure is called with a Project, ContextName and Configuration, it adds a CoverageTask for JVM, while filtering unregistered Dependencies`() {
        // Given
        val project: Project = mockk()
        val projectName: String = fixture()
        val contextName: String = fixture()
    ...
    @Test
    fun `Given configure is called with a Project, ContextName and Configuration, it adds a CoverageTask for Android, while filtering unregistered Dependencies`() {
        // Given
        val project: Project = mockk()
        val projectName: String = fixture()
        val contextName: String = fixture()
